            ...
            try:
                ...
                while connection_active:
                    try:
                        ...
                        # 检查连接是否超时
                        current_time = asyncio.get_event_loop().time()
                        if current_time - last_activity_time > 300:  # 5分钟无活动则断开
                            logger.warning("连接超时，无活动时间过长，主动断开连接")
                            connection_active = False
                            break
...
if __name__ == "__main__":  # 添加了空行以符合 PEP8
    ...
    uvicorn.run(
        ...
        host="0.0.0.0",
        port=8006,
        ws_ping_interval=30,  # 保持较低的ping间隔
        ws_ping_timeout=120,  # 增加ping超时时间
        timeout_keep_alive=300,  # 增加保持连接活跃的超时时间
